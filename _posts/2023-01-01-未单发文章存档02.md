...
In our formmer article, we discussed the anchor which sets a primary standard in our system of evaluation.



I'd like to call it the bridge in this easy. 
...
As a metaphor,  the bridge connects two solated (whether it actually is or seems to be isolated) things together. If retouched with this definition,  what is a language ? The bridge between the inner mind and the outer world. Then what is the difference between languages? The shape, materials, design and location ?



This is what we see from the outside, if watched from the inside, the differences would be the significant. The difference of languages then should be  the difference of perspectives. And the language starts from the starting point of perspective. And we must all find that the starting point is another anchor. Here comes a new anchor. 



Anchor after anchor, anchor it might be. Anchor becomes bridge, bridge ends as a new anchor.  An infinite loop.
...
Firstly we should know things start from our side familiarly, then can we make a valuation about the other side based on experience of ours, then try to launch the fundamental structure towards that direction. The difference between the bridge in reality is that we can try many times which insure the final result with unlimited practices and improvements. (Bridge design is calculated by computer with unlimited computational power, the process is also repeatable and practiced, which makes this hands-on similar somehow )



while learning a language, our regular way is to slide along the learning curve, just the way we did in learning mathematic, physics, or chemistry. It is a great way for learning measuring by standards both in efficiency and effects, and of course following the practice thus facing less pressures at least from the beginning.



However, different from learning the sciences, we have some forgotten ways in language learning. Think about it, we learn the language from the very beginning. In our early childhood, we didn't learn. We feel it,  inmitate what ever we heard, tried any words or sounds we could cry out,  felt it with all our patience and concentration, though being no awareness on that fact. Usually we don't treat our language practicing in childhood as learning. It is more like tree's growing or animal's aging in our oncept. 



It may also be shaped by our minds about the concept learning. Once identified as learning, we rolled back our sleeves, stopped ourselves from being wild, and acted in manners as if we were the gentlemen/women since the birth of human. This time we are limited by the already handled language.



A bridge should be a bridge, or man might think the bridge were the only promised land, and the left parts of the world were ruined by the great flood.



From an other viewpoint, the environment shapes our mind, our mind changes the outer world by perseiving it in return, and vice versa. We got the infinite loop once again.



And once again, how can we stop the loop and find a place setting an anchor. Here comes the most important point, we could find the real anchor only when we were being aware of our ways of thought. The best method here is to know the fact in the first glimpse. but it is as difficult as knowing yourself. we all know that knowing oneself is a process lifelong as a measure.



If  you care only about language, you can ignore all the above things and focus only on things below.



Let's act just like a baby while learning the language.
